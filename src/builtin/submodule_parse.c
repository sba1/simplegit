...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_commands; i++) {
        ...
        if (!strcmp(command->name, "init")) {
            args->init = command->value;
        } else if (!strcmp(command->name, "update")) {
            args->update = command->value;
        }
    }
    /* arguments */
...
static DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    DocoptArgs args = {
        0, 0, 0,
    ...
    Elements elements = {2, 0, 1, commands, arguments, options};
